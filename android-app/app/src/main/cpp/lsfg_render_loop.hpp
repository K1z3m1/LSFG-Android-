...
//   pushFrame(ahb)— runs the whole capture → framegen → blit pipeline
//                   synchronously, on the calling thread: copies into the
//                   next ping-pong slot, presents, and blits outputs before
//                   returning. No queue, no mailbox, no worker thread.
...
// Push a capture frame. Runs the entire import → framegen/AI → blit
// pipeline synchronously on the calling thread and returns only once that
// frame has been fully processed — there is no queue, mailbox, or worker
// thread in between. `timestampNs` should come from Image.getTimestamp()
// when available (used only for the capture-to-display latency stat).
// Safe to call from any thread; if a previous pushFrame() call is still
// running when a new one arrives, the new call blocks until it's done.
