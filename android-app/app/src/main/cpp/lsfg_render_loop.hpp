...
//   pushFrame(ahb)— hands the frame to a single-slot mailbox (overwriting
//                   whatever hasn't been picked up yet) and returns
//                   immediately; a dedicated worker thread copies it into
//                   the next ping-pong slot, presents, and blits outputs.
//                   No backlog queue, no content/Hz-based filtering — at
...
struct RenderLoopConfig {
    ...
    // Display-stage post-processing. These run only when enabled and are
    // deliberately downstream of frame generation: the LSFG/AI interpolation
    // loop and its timing/pacing are not changed by these switches.
    bool gpuPostProcessing;
    float gpuUpscaleFactor; // 1.0 or 1.5 or 2.0
    int gpuMethod;          // 0 = FSR1 EASU+RCAS (only production default)
    float gpuSharpness;
    float gpuStrength;
    bool npuPostProcessing; // NNAPI dedicated accelerator only; no fallback
    int npuPreset;          // 4 = GAME_CRISP
    bool cpuPostProcessing;
    int cpuPreset;          // 5 = GAMER_SHARP
    float cpuStrength;
    float cpuSaturation;
    float cpuVibrance;
    float cpuVignette;
...
// Push a capture frame. Hands off to a single-slot mailbox and returns
// immediately — does not block on GPU work. A dedicated worker thread
// drains the mailbox and runs the import → framegen/AI → blit pipeline;
// if a new frame arrives before the worker picks up the previous one, the
// previous one is dropped (only ever one frame in flight, never a
// backlog). `timestampNs` should come from Image.getTimestamp() when
// available (used only for the capture-to-display latency stat).
