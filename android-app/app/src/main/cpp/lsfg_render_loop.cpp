...
struct State {
    ...
    // Single-slot mailbox, not a backlog queue: at most one frame is ever
    // waiting to be picked up. pushFrame() overwrites this slot (dropping
    // whatever hasn't been consumed yet, if anything) and returns
    // immediately — it never blocks on GPU work. A dedicated worker thread
    // drains it, always processing whatever is newest. This indirection is
    // what lets the capture thread stay responsive (closing Images / calling
    // acquireLatestImage promptly) instead of stalling behind waitIdle for
    // the whole pipeline duration — losing that overlap was what made
    // latency worse when this was briefly made fully synchronous. There is
    // still no backlog and no content/Hz-based filtering: exactly one frame,
    // always the latest, is in flight at a time.
    struct MailboxFrame {
    ...
    std::optional<MailboxFrame> mailbox;
...
// Applies big-core affinity + elevated scheduling priority to the render
// worker thread. Called once, at the top of workerThread() — there's a
// single persistent worker again, not a per-capture-thread tuning latch.
void applyWorkerThreadTuning() {
    // Prefer big/performance cores for every CPU-side operation in this hot
    // thread. This is thread-local affinity; it does not require root and does
    // not force unrelated application threads onto the big cluster.
    if (!g.cpuPolicy.useBigCores()) {
        LOGW("workerThread: big-core affinity unavailable; using all online CPUs");
...
//   queue    — time the frame sat in the single-slot mailbox before the
//              worker picked it up. Near-zero in practice (the worker is
//              usually already waiting), but not fixed at 0: pushFrame()
//              hands off and returns immediately rather than blocking on
//              GPU work, which is what keeps the capture thread free to
//              keep draining ImageReader instead of stalling behind
//              waitIdle every frame.
...
// Processes exactly one capture frame: import → copy into the ping-pong
// slot → framegen (or AI) → blit every output. Runs on the dedicated
// worker thread only, one frame at a time — never called concurrently.
// `queuedAt` is when pushFrame() handed this frame off, used only for the
// queueNs profiling stat above.
void processFrame(AHardwareBuffer *ahb, int64_t captureTimestampNs,
                   State::Clock::time_point queuedAt) {
    // Persist the rolling profiling window and the reusable empty-semaphore
    // vector across calls — this now runs in a persistent worker thread
    // loop rather than being freshly entered per capture callback, but is
    // still kept static rather than a workerThread()-local so the extraction
    // into a standalone function stays clean.
    ...
    prof.queueNs += std::chrono::duration_cast<std::chrono::nanoseconds>(
        frameWorkStartedAt - queuedAt).count();
    ...
    if (!srcFromCache) {
        ...
        if (rc != kOk) {
            ...
            AHardwareBuffer_release(ahb); // release worker-mailbox ref
            return;
        }
    ...
    // the mailbox ref that pushFrame() acquired for the handoff to this
    // worker thread.
    if (srcFromCache) {
    ...
    if (runFramegen || runAi) {
        ...
        // Note: prof has static storage duration (declared static above), so
        // it must NOT appear in the capture list — only automatic-duration
        // locals need capturing; prof is referenced directly in the body.
        auto timedBlit = [&blitWorkNsThisFrame, captureTimestampNs](const AhbImage &out) {
...
// Persistent render worker: applies CPU tuning once, then loops picking the
// newest frame out of the single-slot mailbox and running it through
// processFrame(). This is what lets pushFrame() hand off and return
// immediately instead of blocking the capture thread behind GPU work.
void workerThread() {
    applyWorkerThreadTuning();

    while (true) {
        State::MailboxFrame frame{};
        {
            std::unique_lock<std::mutex> lock(g.frameMu);
            g.frameCv.wait(lock, []{ return g.stopRequested || g.mailbox.has_value(); });
            if (g.stopRequested && !g.mailbox.has_value()) return;
            frame = *g.mailbox;
            g.mailbox.reset();
        }
        if (frame.ahb == nullptr) continue;
        processFrame(frame.ahb, frame.captureTimestampNs, frame.queuedAt);
    }
}
...
void pushFrame(AHardwareBuffer *ahb, int64_t timestampNs) {
    ...
    // Single-slot mailbox hand-off: overwrite whatever's waiting (there's
    // at most one, and if the worker hasn't picked it up yet it's already
    // stale) with this frame, and return immediately. This is what keeps
    // the capture thread free to close Images / call acquireLatestImage
    // promptly instead of blocking behind the worker's GPU work — that
    // overlap is what actually keeps latency down; briefly making this
    // call synchronous made latency worse, not better, because it stalled
    // capture behind waitIdle every single frame. Still exactly one frame
    // in flight, still no backlog, still no content/Hz-based filtering.
    AHardwareBuffer_acquire(ahb);
    {
        ...
        if (g.mailbox.has_value()) {
            AHardwareBuffer_release(g.mailbox->ahb);
        }
        g.mailbox = State::MailboxFrame{
            .ahb = ahb,
            .captureTimestampNs = timestampNs,
            .queuedAt = State::Clock::now(),
        };
    }
    g.frameCv.notify_one();
}

void shutdownRenderLoop() {
    // Signal the worker to stop and wake it, then join it — mirrors the
    // original mailbox+worker-thread design. Joining here (before touching
    // Vulkan/framegen state below) guarantees the worker isn't mid-frame
    // when we start tearing things down.
    {
        std::lock_guard<std::mutex> lock(g.frameMu);
...

    {
        ...
        // Release whatever's left sitting in the mailbox (the worker exits
        // its wait loop without draining it once stopRequested is set).
        {
