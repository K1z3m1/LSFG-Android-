...
void workerThread() {
    ...
    // submit→wait→blit pipeline and must wake up close to the vsync deadlines
    // computed by sleepUntilVsyncAligned(). At the default priority (SCHED_OTHER,
    // nice 0) it competes evenly with every other thread on the device — the
    // captured app's own render thread, GC, system services — any of which can
    // delay its wakeup and cause a late/dropped present regardless of how
    // accurate the deadline math is.
    //
    ...
    const std::vector<int> noOutputSemaphores{};
