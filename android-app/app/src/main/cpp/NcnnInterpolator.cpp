...
int NcnnInterpolator::load(const std::string &modelDir, bool allowGpu, int vulkanDeviceIndex, int numThreads) {
    ...
    (void)numThreads; // CPU thread count is controlled by the adaptive big-core policy.
    ncnnCpuSetBigFirst();
    const int kUtilityThreads = ncnnCpuBigThreadCount();
    ...
    LOGI("ncnn RIFE model loaded from %s (mode=%s, bigCoreThreads=%d, allCoreThreads=%d, winograd=%s)", modelDir.c_str(),
         "GPU-only", ncnnCpuBigThreadCount(), ncnnCpuAllThreadCount(), "off");
    return kNcnnOk;
}
