...
class OverlayManager(private val ctx: Context) {
    ...
    fun show() {
        ...
        // Output pacing itself is intentionally uncapped (no software sleep/throttle
        // anywhere in the render path — see lsfg_render_loop.cpp). But an uncapped
        // *producer* still gets silently capped at the *consumer* end: Choreographer
        // only fires doFrame() once per VSYNC of whatever display mode is currently
        // active, and Android's power-saving default is commonly 60 Hz even on panels
        // that support 90/120/144/165/240 Hz. Below we vote for the highest-Hz mode
        // at the overlay's resolution so that implicit cap doesn't silently override
        // the uncapped pacing above.
        ...
        val params = WindowManager.LayoutParams(
            ...
            // Vote for the panel's highest available refresh rate at this resolution
            // (60/90/120/144/165/240 Hz+, whatever the device actually supports).
            // WindowManagerService's DisplayModeDirector aggregates preferredDisplayModeId
            // votes from every visible window on the display, not just the focused one,
            // so this works for a FLAG_NOT_FOCUSABLE overlay same as it would for a normal
            // activity. 0 (the fallback below) means "no preference / system default" and
            // is always a safe value to assign.
            preferredDisplayModeId = pickHighestRefreshRateModeId(wm, screenW, screenH)
            @Suppress("DEPRECATION")
        ...
        tex.surfaceTextureListener = object : TextureView.SurfaceTextureListener {
            override fun onSurfaceTextureAvailable(st: SurfaceTexture, width: Int, height: Int) {
                ...
                // The Choreographer poll above only picks up a ready buffer on the
                // *next* VSYNC tick, even if native finished queuing it right after
                // the previous tick — up to one whole VSYNC interval of pure waiting
                // for no reason, on top of whatever Hz the display is already voted
                // to. Registering our own listener invalidates the instant a buffer
                // actually lands instead of waiting for the poll, so on the (most)
                // devices where SurfaceTexture callbacks work normally, pickup
                // latency drops close to zero instead of averaging half a VSYNC
                // period. postInvalidateOnAnimation() is safe to call from whatever
                // arbitrary thread this listener fires on. The Choreographer loop
                // keeps running unconditionally alongside this as the fallback for
                // the devices where onFrameAvailable doesn't fire at all — redundant
                // invalidate() calls just collapse into a single draw pass, so
                // running both mechanisms together is harmless.
                st.setOnFrameAvailableListener({ tex.postInvalidateOnAnimation() })
            }
    ...
    /**
     * Finds the highest-refresh-rate [android.view.Display.Mode] at (or closest to) the
     * given resolution, so the overlay window can vote for a rate above whatever the
     * system currently defaults to. Without this vote, [Choreographer] fires once per
     * VSYNC of the *active* mode only — so even though nothing downstream sleeps or
     * throttles, output silently gets capped at 60 fps on 90/120/144/165/240 Hz-capable
     * panels that just haven't been asked to switch modes.
     *
     * `Display.getSupportedModes()` reports physical resolutions in the display's
     * natural orientation regardless of current rotation, so matching checks both
     * `(w,h)` and the rotated `(h,w)` against the target size. Falls back to scanning
     * every mode (ignoring resolution) if no exact match is found, so a real mode is
     * still picked rather than silently doing nothing.
     *
     * Returns 0 ("no preference / system default") on any failure or if the API/mode
     * list is unavailable — always safe to assign directly to
     * [WindowManager.LayoutParams.preferredDisplayModeId].
     */
    private fun pickHighestRefreshRateModeId(wm: WindowManager, targetW: Int, targetH: Int): Int {
        return runCatching {
            @Suppress("DEPRECATION")
            val modes = wm.defaultDisplay?.supportedModes
            if (modes == null || modes.isEmpty()) return@runCatching 0

            val sameRes = modes.filter {
                (it.physicalWidth == targetW && it.physicalHeight == targetH) ||
                    (it.physicalWidth == targetH && it.physicalHeight == targetW)
            }
            val candidates = sameRes.ifEmpty { modes.toList() }
            val best = candidates.maxByOrNull { it.refreshRate } ?: return@runCatching 0
            Log.i(TAG, "Refresh-rate vote: ${best.refreshRate} Hz (modeId=${best.modeId}, " +
                "${best.physicalWidth}x${best.physicalHeight}) — ${modes.size} modes available")
            best.modeId
        }.onFailure {
            Log.w(TAG, "pickHighestRefreshRateModeId failed, using system default", it)
        }.getOrDefault(0)
    }
